...
# merge Pepsi and Cola prices for the 500ml colums to have the price of cola
da1_data$cola_price_500ml <- with(da1_data,ifelse(is.na(coke_price_500ml),pepsi_price_500ml,coke_price_500ml))


# To have less missing values calculte the estimated price of a 500ml cola based on the prices of smaller or larger quantities (assuming price/ml is the same)
...
#Delete unnecesarry columns as we have the data in the merged 
...
## Distribution of real 500ml Cola prices
...
ggplot(data=da1_data, aes(x=cola_price_500ml))+
  ...
  labs(x="Price of Cola", y="Density") +
  theme_bw()+
  xlim(190,1200)
...
## Distribution of estimated 500ml Cola prices
```{r, echo=FALSE, out.width = '30%', warning=FALSE}

ggplot(data=da1_data, aes(x=cola_estimated_price_500ml))+
  geom_histogram( aes(y = ..density..), fill="chocolate4", binwidth = 40)+
  labs(x="Price of Cola", y="Density") +
  theme_bw()+
  xlim(190,1200)

ggplot(data=da1_data, aes(x=cola_estimated_price_500ml))+
  geom_density(aes(y=..density..), fill="chocolate", alpha=0.4) +
  labs(x="Price of Cola", y="Density") +
  theme_bw()
...
## Prices of Margarita pizza in Budepest vs Rest of Hungary
```{r, echo=FALSE, out.width = '50%', warning=FALSE}
da1_data %>% 
  ggplot(aes(x=margarita_price, fill=city))+
  geom_density(aes(y=..density..), alpha=0.1) +
  labs(x="Prices of Margarity pizza", y="Density", fill="Budapest or Rest of Hungary") +
  theme_bw()

da1_data %>% 
  ggplot(aes(x=margarita_price, color=city))+
  geom_histogram( aes(y = ..density..), fill="chocolate4", binwidth = 100)+
  labs(x="Prices of Margarity pizza", y="Density", fill="Budapest or Rest of Hungary") +
  theme_bw()
...
price_vs_size <- ggplot( da1_data , aes( x = margarita_price , y = margarita_size ) )+
    ...
    labs( x = 'Price of Margarita Pizza' , y = 'Size of Margarita Pizza') + geom_vline(xintercept = mean_price_32[[1]], linetype = "dashed") + geom_vline(xintercept = mean_price_30[[1]], color = "red", linetype = "dashed")
